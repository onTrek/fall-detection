...
class CoarseFineGRU(pl.LightningModule):
    def __init__(self, num_classes=2, lr=0.001, dropout=0.2, weight=[1.0, 1.0], input_shape=(3, 140)):
        super().__init__()
        self.save_hyperparameters()

        self.coarse = CoarseBranch()
        self.fine = FineBranch(dropout=dropout)
        self.temporal = TemporalBranch(input_size=input_shape[0])
        ...
        total_dim = coarse_dim + fine_dim + temporal_dim
    ...
    def forward(self, x):
        ...
        f_temp = self.temporal(x)

        concat = torch.cat((f_coarse, f_fine, f_temp), dim=1)
