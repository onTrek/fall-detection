...
class HybridCNNBiLSTMModel(pl.LightningModule):
    def __init__(self, input_len=100, num_classes=2, lr=0.001, dropout=0.3, weights=[1.0, 1.0]):
        super().__init__()
        ...
        self.acc_cnn = nn.Sequential(
            ...
            nn.ReLU(),
            nn.AdaptiveAvgPool1d(output_size=input_len),
        )
        ...
        combined_dim = (64 * input_len) + (128 * 2 * input_len)
    ...
    def forward(self, x):
        ...
        acc_feat = acc_feat.view(acc_feat.size(0), -1)


        gyro_feat, _ = self.gyro_lstm(gyro)
        gyro_feat = gyro_feat.contiguous().view(gyro_feat.size(0), -1)
